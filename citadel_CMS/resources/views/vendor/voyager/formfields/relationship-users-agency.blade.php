            ...
            @else
                <select class="form-control select2" name="{{ $options->column }}">
                    @php
                        ...
                        if ($dataTypeContent->role_id == 2)//Admin
                            $query = $model::select()->where('role_id', 1)->get();

                        else if ($dataTypeContent->role_id == 3)//Management
                            $query = $model::select()->where('role_id', 2)->get();

                        else if ($dataTypeContent->role_id == 4 || $dataTypeContent->role_id == 6)//Sales manager or Installer manager
                            $query = $model::select()->where('role_id', 3)->get();

                        else if ($dataTypeContent->role_id == 5)//Salesperson
                            $query = $model::select()->where('role_id', 4)->get();

                        else if ($dataTypeContent->role_id == 7)//Salesperson
                            $query = $model::select()->where('role_id', 6)->get();
