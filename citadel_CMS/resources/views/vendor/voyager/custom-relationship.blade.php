                ...
                @if(isset($query))
                        @if($row->display_name == "File Name")
                            <div class="file-name-container">
                                @php
                                    $fullFileName = $query->{$options->label}; // Full filename
                                    $displayFileName = strlen($fullFileName) > 30 ? substr($fullFileName, 0, 15) . '...' . substr($fullFileName, -10) : $fullFileName;
                                @endphp
                                <!-- Truncated file name with hover to see full name -->
                                 
                                <p class="file-name" title="{{ $fullFileName }}">{{ $displayFileName }}</p>
                                <a href="{{ Storage::disk(config('voyager.storage.disk'))->url($query->upload_document) }}" class="download-link" target="_blank">
                                    {{ __('voyager::generic.download') }}
                                </a>
                            </div>
                        @else
                            @php
                                // Change values based on the specified conditions
                                $labelValue = $query->{$options->label};
                                switch ($labelValue) {
                                    case "PENDING":
