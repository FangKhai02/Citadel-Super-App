...
<div class="container-fluid">
    ...
    @can('delete', app($dataType->model_name))
    @if($usesSoftDeletes)
    <input type="checkbox" @if ($showSoftDeleted) checked @endif id="show_soft_deletes" data-toggle="toggle" data-on="{{ _('voyager::bread.soft_deletes_off') }}" data-off="{{ _('voyager::bread.soft_deletes_on') }}">
    @endif
    @endcan
                        ...
                        <table id="dataTable" class="table table-hover">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    {{ trans_choice('voyager::media.files', 0) }}
                                    @endif
                                    @elseif($row->type == 'multiple_file' && !empty($data->{$row->field}))
                                    @if(json_decode($data->{$row->field}) !== null)
                                        @foreach(json_decode($data->{$row->field}) as $file)
...
<script>
    ...
    $('td').on('click', '.delete', function (e) {
        $('#delete_form')[0].action = '{{ route('voyager.'.$dataType->slug.'.destroy', '_id') }}'.replace('_id', $(this).data('id'));
