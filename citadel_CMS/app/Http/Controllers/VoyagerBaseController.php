...
class VoyagerBaseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($slug === 'product-type' && ProductType::where('name', $request->input('name'))->exists()) {
    ...
    public function destroy(Request $request, $id)
    {
        ...
        foreach ($ids as $id) {
            ...
            // Check if ProductType is linked to a TrustProduct
            if ($dataType->slug === 'product-type' && $this->isLinkedToTrustProduct($data)) {
                // Prevent deletion if linked to a TrustProduct
                return redirect()->route("voyager.{$dataType->slug}.index")
                    ->with([
                        'message' => __('You are not allowed to delete a Product Type linked to a TrustProduct.'),
                        'alert-type' => 'error',
                    ]);
            }

            // Check if DocumentType is linked to a ProductAgreement
            if ($dataType->slug === 'document-type' && $this->isLinkedToProductAgreement($data)) {
                // Prevent deletion if linked to a ProductAgreement
                return redirect()->route("voyager.{$dataType->slug}.index")
                    ->with([
                        'message' => __('You are not allowed to delete a Document Type linked to a Product Agreement.'),
                        'alert-type' => 'error',
                    ]);
            }
            ...
            // Check if the record is a 'Client' or 'Agent' and delete accordingly
            if ($dataType->slug === 'client' || $dataType->slug === 'agent' || $dataType->slug === 'corporate-client') {
                if ($data->app_user_id) {
                    // Conditional deletion based on the type
                    if ($dataType->slug === 'client') {
                        $clientController = new ClientController();
                        return $clientController->deactivateClient($data->id);
                    } 
                    elseif ($dataType->slug === 'corporate-client') {
                        $corporateClientController = new CorporateClientController();
                        return $corporateClientController->deactivateCorporateClient($data->id);
                    }     
                    elseif ($dataType->slug === 'agent') {
                        DB::transaction(function () use ($data) {
                            Client::where('agent_id', $data->id)->update(['agent_id' => null]);
                            Agent::where('recruit_manager_id', $data->id)->update(['recruit_manager_id' => null]);
                            AppUserSession::where('app_user_id', $data->app_user_id)->delete();
                            Agent::where('app_user_id', $data->app_user_id)->delete();
                            // Delete the user_detail record if exists
                            if ($data->user_detail_id) {
                                UserDetails::where('id', $data->user_detail_id)->delete();
                            }
                            // Delete the app_user record after removing references
                            AppUser::where('id', $data->app_user_id)->delete();
                        });
                    }
    ...
    public function relation(Request $request)
    {
        ...
        foreach ($rows as $key => $row) {
            if ($row->field === $request->input('type')) {
                ...
                if ($model->getModel()->getTable() == 'agent_role_settings')
                    $results = array_values(array_filter($results, function ($item) {
                        return $item['text'] !== 'Peer to Peer';
                    }));
    ...
    protected function isLinkedToTrustProduct($productType)
    {
        // Check if the ProductType is linked to any TrustProducts
        return $productType->product()->exists();
    }

    protected function isLinkedToProductAgreement($documentType)
    {
        // Check if the DocumentType is linked to any ProductAgreements
        return $documentType->productAgreements()->exists();
    }
}
