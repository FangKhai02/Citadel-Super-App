...
use App\Models\ProductTargetReturn;
use App\Util\Util;
...
class ProductTargetReturnController extends CustomVoyagerBaseController
{
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            // Modification by Thives to filter by product id if redirected from product browse
    ...
    public function destroy(Request $request, $id)
    {
        ...
        foreach ($ids as $id) {
            ...
            if ($dataType->slug === 'corporate-client') {
                $corporateController = new CorporateClientController();
                return $corporateController->deleteCorporate($data->id);
            }
        ...
        $redirect = redirect()->route('voyager.product-target-return.index', ['key' => 'product_id',
        'filter' => 'contains',
        's' => $data->product_id]);
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$request->ajax()) {
            ...
            $productTargetReturn = ProductTargetReturn::findOrFail($data->id);
            if ($productTargetReturn->condition_type == 'ABOVE' ||  $productTargetReturn->condition_type == 'BELOW') {
                $productTargetReturn->minimum = null;
                $productTargetReturn->maximum = null;
                $productTargetReturn->save();
            } elseif ($productTargetReturn->condition_type == 'RANGE') {
                $productTargetReturn->threshold_amount = null;
                $productTargetReturn->save();
            }
            ...
            if (auth()->user()->can('browse', app($dataType->model_name))) {
                ...
                // update redirect to route 'route' => route('voyager.product-target-return.index', ['key' => 'product_id',
                    // 'filter' => 'contains',
                    // 's' => $row->id])
                $redirect = redirect()->route('voyager.product-target-return.index', ['key' => 'product_id',
                    'filter' => 'contains',
