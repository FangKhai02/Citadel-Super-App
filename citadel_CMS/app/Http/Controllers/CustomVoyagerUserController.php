...
class CustomVoyagerUserController extends VoyagerBaseController
{
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($search->value != '' && $search->key && $search->filter) {
                ...
                if ($row = $this->findSearchableRelationshipRow($dataType->rows->where('type', 'relationship'), $search->key)) {
                    ...
                    if ($searchField === "users.user_belongsto_role_relationship") {
                        $query->whereHas('role', function ($query) use ($search_value) {
                            $query->where('display_name', 'LIKE', "%{$search_value}%");
                        });
                    }else if ($searchField === "users.user_belongsto_user_relationship") {
                        $query->whereHas('creator', function ($query) use ($search_value) {
                            // Search by the creator's name
    ...
    public function store(Request $request)
    {
        ...
        if ($user) {
            return redirect()->back()
                ->withInput()
                ->with([
                    'message' => 'Email has been used by another user, please try again with another email.',
                    'alert-type' => 'error',
                ]);
        }
        ...
        if (!$request->ajax()) {
            ...
            if (!$request->has('_tagging')) {
                ...
                return $redirect->with([
                    'message' => 'Successfully added new ' . strtolower($dataType->getTranslatedAttribute('display_name_singular')),
                    'alert-type' => 'success',
                ]);
            } else {
    ...
    public function update(Request $request, $id)
    {
        ...
        // Validate fields with ajax (if applicable)
