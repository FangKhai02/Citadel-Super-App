...
use App\Database\Schema\CustomSchemaManager;
use Illuminate\Database\Eloquent\SoftDeletes;
use App\Http\Controllers\Custom\CustomController;
use TCG\Voyager\Http\Controllers\Traits\BreadRelationshipParser;

class CustomVoyagerBaseController extends CustomController
{
...
//            }
    ...
    public function relation(Request $request)
    {
        ...
        foreach ($rows as $key => $row) {
            if ($row->field === $request->input('type')) {
                ...
                if (!$search && $page == 1 && $options->type == 'belongsToMany') {
                    $results[] = [
                        'id' => 'all',
                        'text' => 'Select All',
                    ];
    ...
    /**
     * Get isEsim boolean ('1'/'0') based on string 'true'/'false'
     *
     * @param $search
     * @param $search_filter
     * @param $search_value
     ...
     */
    private function handleIsEsimSearchValue($search, $search_filter, $search_value)
    {
        if ($search->key == 'isEsim') {
            if ($search_filter == '=') {
                switch ($search->value) {
                    case 'eSIM':
                        $search_value = 1;
                ...
                if (str_contains(strtolower('eSIM'), strtolower($search->value))) {
                    $search_value = 1;
                } else if (str_contains(strtolower('Physical SIM'), strtolower($search->value))) {
                    $search_value = 0;
                } else {
                    $search_value = '';
                }
