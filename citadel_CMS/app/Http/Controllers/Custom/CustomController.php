...
use App\Http\Controllers\ContentType\MultipleFile;
...
abstract class CustomController extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        foreach ($rows as $row) {
             ...
             */
            if (in_array($row->type, ['multiple_images', 'file', 'multiple_file']) && !is_null($content)) {
            ...
            if (is_null($content)) {
                ...
                // If the multiple_file upload is null and it has a current file keep the current file
                if ($row->type == 'multiple_file' && is_null($request->input($row->field)) && isset($data->{$row->field})) {
                    $content = $data->{$row->field};
                    if (!$content) {
                        $content = json_encode([]);
                    }
    ...
    public function getContentBasedOnType(Request $request, $slug, $row, $options = null)
    {
        switch ($row->type) {
            ...
            /********** MULTIPLE FILE TYPE **********/
            case 'multiple_file':
                return (new MultipleFile($request, $slug, $row, $options))->handle();
